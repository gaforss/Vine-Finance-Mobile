    ...
    <Modal visible={visible} animationType="slide" transparent>
      <View style={styles.modalOverlay}>
        <View style={styles.modalContent}>
          <Text style={styles.modalTitle}>{form._id ? 'Edit' : 'Add'} Goal</Text>
          <TextInput style={styles.input} placeholder="Name" value={form.name || ''} onChangeText={v => setForm(f => ({ ...f, name: v }))} />
          <TextInput style={styles.input} placeholder="Target Amount" keyboardType="numeric" value={form.targetAmount?.toString() || ''} onChangeText={v => setForm(f => ({ ...f, targetAmount: parseFloat(v) }))} />
          <TextInput style={styles.input} placeholder="Current Amount" keyboardType="numeric" value={form.currentAmount?.toString() || ''} onChangeText={v => setForm(f => ({ ...f, currentAmount: parseFloat(v) }))} />
          <TextInput style={styles.input} placeholder="End Date (YYYY-MM-DD)" value={form.endDate?.toString().slice(0,10) || ''} onChangeText={v => setForm(f => ({ ...f, endDate: v }))} />
...
const SavingsGoalsScreen = () => {
  ...
  const handleDelete = (id: string) => {
    Alert.alert('Delete', 'Are you sure?', [
      ...
      { text: 'Delete', style: 'destructive', onPress: async () => { await apiService.deleteSavingsGoal(id); loadGoals(); } }
    ]);
  };
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: '#181f2a' }}>
      ...
      <FlatList data={goals} keyExtractor={item => item._id!} renderItem={({ item }) => (
        <View style={styles.row}>
          <View style={{ flex: 1 }}>
            <Text style={styles.rowText}>{item.name}</Text>
            <Text style={styles.rowSub}>Target: ${item.targetAmount.toFixed(2)} | Current: ${item.currentAmount.toFixed(2)}</Text>
            <Text style={styles.rowSub}>End: {item.endDate?.toString().slice(0, 10)}</Text>
          </View>
      ...
      <GoalForm visible={modalVisible} onClose={() => { setModalVisible(false); setEditing(null); }} onSave={handleSave} initialData={editing} />
    </SafeAreaView>
  );
};
