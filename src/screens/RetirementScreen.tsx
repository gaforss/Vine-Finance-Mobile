...
const RetirementScreen: React.FC = () => {
  ...
  const [projections, setProjections] =
    useState<RetirementProjectionsResult | null>(null);
  const [comparison, setComparison] = useState<NetWorthComparisonResult | null>(
    null,
  );
  ...
  const totalPercent = CATEGORY_FIELDS.reduce(
    (sum, cat) => sum + (Number(form[cat.key as keyof RetirementGoals]) || 0),
    0,
  );
  ...
  const handleSave = async () => {
    if (totalPercent !== 100) {
      Alert.alert(
        'Error',
        'The total percentage of all categories must equal 100%.',
      );
      return;
    }
    setSaving(true);
  ...
  // Chart data
  const chartData =
    projections?.projections?.find(p => Math.abs(p.rate - 7) < 0.01) ||
    projections?.projections?.[0];
  const chartSeries =
    chartData?.data?.map(d => ({x: d.year, y: d.value})) || [];
      ...
      <ScrollView contentContainerStyle={styles.scrollContent}>
        ...
        <View style={styles.card}>
              ...
              fill={
                comparison
                  ? Math.round(
                      Math.min(
                        100,
                        (comparison.userNetWorth /
                          (comparison.ageGroupAverage || 1)) *
                          100,
                      ),
                    )
                  : 0
          ...
          <View style={[styles.outcomeBox, {backgroundColor: '#153e2e'}]}>
            ...
            <Text style={styles.outcomeText}>
              Excellent! You're exceeding the top 10% of your peers and are on
              track to meet your retirement goals. Your financial planning is
              outstanding.
            </Text>
        ...
        <View style={styles.card}>
          <Text style={styles.cardTitle}>
            How Am I Tracking?{' '}
            <Text style={{color: '#4fc3f7'}}>| Review Goals</Text>
          </Text>
          <VictoryChart
            domainPadding={20}
            height={260}
        ...
        <View
          style={[
            ...
            {
              alignItems: 'center',
              backgroundColor: goalMet ? '#153e2e' : '#3e1a15',
              borderColor: goalMet ? '#28a745' : '#dc3545',
            },
          ]}>
          <Text style={goalMet ? styles.outcomeCheck : styles.outcomeWarn}>
            {goalMet ? '✅' : '⚠️'}
          </Text>
          <Text style={styles.outcomeMain}>
            {goalMet
          ...
          <Text style={styles.outcomeSub}>
            {goalMet
              ? 'Based on your current plan, you can retire by age'
              : "Based on your current plan, you're projected to have a shortfall by your target retirement age."}
          </Text>
          <Text style={goalMet ? styles.outcomeAge : styles.outcomeShortfall}>
            {goalMet ? outcomeAge : `$${shortfall?.toLocaleString()}`}
          </Text>
          <Text style={styles.outcomeFooter}>
            {goalMet
              ? 'Keep up the great work! Consider reviewing your goals periodically to stay on course.'
              : 'Consider increasing your savings, adjusting your retirement age, or reducing your spending to close the gap.'}
          </Text>
        </View>
        ...
        <View style={styles.card}>
          ...
          <Text style={styles.cardDesc}>
            Select a template to automatically adjust your budget allocations
            based on your preferred retirement lifestyle. Each template balances
            essential expenses with lifestyle spending differently.
          </Text>
          {Object.entries(TEMPLATES).map(([key, template]) => (
            <View
              ...
              style={[
                styles.templateCard,
                key === 'conservative'
                  ? styles.templateConservative
                  : key === 'moderate'
                  ? styles.templateModerate
                  : styles.templateAggressive,
              ]}>
              ...
              <View style={styles.templateList}>
                {Object.entries(template).map(([cat, val]) => (
                  <Text key={cat} style={styles.templateItem}>
                    ✔️ {CATEGORY_FIELDS.find(f => f.key === cat)?.label || cat}:{' '}
                    {val}%
                  </Text>
                ))}
              </View>
              <TouchableOpacity
                ...
                onPress={() => handleTemplate(key as keyof typeof TEMPLATES)}>
                <Text style={styles.templateBtnText}>
                  Apply {key.charAt(0).toUpperCase() + key.slice(1)} Template
                </Text>
              </TouchableOpacity>
            </View>
          ))}
        ...
        <View style={styles.card}>
          <Text style={styles.cardTitle}>
            Your Projected Monthly Budget Based On Retirement Goal
          </Text>
            ...
            <View key={cat.key} style={styles.breakdownRow2}>
              ...
              <View style={{flex: 1}}>
                <Text style={styles.breakdownAmount}>
                  $
                  {form.monthlySpend && form[cat.key as keyof RetirementGoals]
                    ? (
                        (Number(form.monthlySpend) *
                          (Number(form[cat.key as keyof RetirementGoals]) ||
                            0)) /
                        100
                      ).toLocaleString()
                    : '0'}
        ...
        <View style={styles.card}>
          ...
          <View style={styles.formRow}>
            ...
            <TextInput
              ...
              value={form.currentAge?.toString() || ''}
              onChangeText={v => handleInput('currentAge', Number(v))}
              placeholder="e.g. 30"
            />
          </View>
          <View style={styles.formRow}>
            ...
            <TextInput
              ...
              onChangeText={v => handleInput('retirementAge', Number(v))}
              placeholder="e.g. 65"
            />
          </View>
          <View style={styles.formRow}>
            ...
            <TextInput
              style={styles.input}
              keyboardType="numeric"
              value={form.annualSavings?.toString() || ''}
              onChangeText={v => handleInput('annualSavings', Number(v))}
          ...
          <View style={styles.formRow}>
            ...
            <TextInput
              style={styles.input}
              keyboardType="numeric"
              value={form.monthlySpend?.toString() || ''}
              onChangeText={v => handleInput('monthlySpend', Number(v))}
              placeholder="e.g. 7500"
            />
          </View>
          <Text
            style={{
              color: totalPercent === 100 ? '#28a745' : '#dc3545',
              fontWeight: 'bold',
              marginBottom: 8,
              textAlign: 'center',
            }}>
          ...
          {CATEGORY_FIELDS.map(cat => (
            <View key={cat.key} style={styles.sliderRow}>
              ...
              <Slider
                ...
                maximumValue={100}
                step={1}
                value={Number(form[cat.key as keyof RetirementGoals]) || 0}
                onValueChange={(v: number) =>
                  handleSlider(cat.key as keyof RetirementGoals, v)
                }
                minimumTrackTintColor="#0070ba"
                maximumTrackTintColor="#ccc"
              />
              <Text style={styles.sliderValue}>
                {Number(form[cat.key as keyof RetirementGoals]) || 0}%
              </Text>
            </View>
          ))}
