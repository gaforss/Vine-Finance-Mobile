...
const BudgetingScreen: React.FC = () => {
  ...
  const loadAllData = useCallback(async () => {
    try {
      ...
      if (hasLinked) {
        ...
        // Fetch detailed transactions for the chart
        ...
        if (transRes.success && Array.isArray(transRes.data)) {
          const timeSeries = buildCashFlowTimeSeries(transRes.data, 30);
        ...
        // If no accounts, clear plaid-dependent data
        setCashFlow(null);
        setSpending([]);
        setGoals([]);
        setCashFlowSeries([]);
  ...
  const generateInsights = () => {
    ...
    if (cashFlow) {
      if (cashFlow.netIncome > 0) {
        insights.push({
          icon: 'smile',
          color: '#4caf50',
          text: `Positive cash flow: You're saving $${cashFlow.netIncome.toLocaleString()} this period.`,
        });
      } else if (cashFlow.netIncome < 0) {
        insights.push({
          icon: 'frown',
          color: '#e57373',
          text: `Negative cash flow: You're spending $${Math.abs(
            cashFlow.netIncome,
          ).toLocaleString()} more than you earn.`,
        });
      } else {
        insights.push({
          icon: 'meh',
          color: '#ffb300',
          text: 'Balanced budget: Your income and expenses are equal.',
        });
      }
    }
    ...
    if (goals && goals.length > 0) {
      ...
      if (completed > 0) {
        insights.push({
          icon: 'flag-checkered',
          color: '#4caf50',
          text: `You've completed ${completed} savings goal${
            completed > 1 ? 's' : ''
          }!`,
        });
      }
    ...
    if (insights.length === 0) {
      insights.push({
        icon: 'info-circle',
        color: '#b0b8c1',
        text: 'Link your accounts to get personalized financial insights.',
      });
    }
    return insights;
  };
  ...
  const chartDataValid =
    Array.isArray(cashFlowSeries) &&
    cashFlowSeries.length > 0 &&
    cashFlowSeries.every(
      d =>
        d &&
        typeof d.income === 'number' &&
        typeof d.expenses === 'number' &&
        typeof d.net === 'number',
    );

  const chartLabels = cashFlowSeries.map(d =>
    d.date ? dayjs(d.date).format('MMM D') : '',
  );
  ...
  const expensesAreaData = expensesData.map((y, i) => ({
    x: chartLabels[i],
    y,
  }));
  ...
  if (cashFlowSeries.length > 0) {
    console.log('chartDataValid:', chartDataValid);
    console.log('cashFlowSeries:', cashFlowSeries);
    ...
    const incomeAreaElem = (
      <VictoryArea
        data={incomeAreaData}
        interpolation="monotoneX"
        style={{
          data: {fill: 'url(#incomeGradient)', stroke: '#23aaff', strokeWidth: 2},
        }}
      />
    );
    const expensesAreaElem = (
      <VictoryArea
    ...
    [incomeAreaElem, expensesAreaElem, netAreaElem].forEach((elem, idx) => {
      console.log(
        `VictoryGroup child[${idx}] type:`,
        typeof elem,
        'isArray:',
        Array.isArray(elem),
        'keys:',
        elem && typeof elem === 'object' ? Object.keys(elem) : 'N/A',
      );
    });
  }
  ...
  const renderHeader = () => (
    <View
      ...
      <View>
        ...
        <Text style={{color: '#b0b8c1', fontSize: 15, marginTop: 2}}>
      ...
      <SafeAreaView
        ...
        <View style={styles.ctaCard}>
          ...
          <Text
            ...
            Connect your accounts to automatically track spending, create
            budgets, and monitor your cash flow.
          </Text>
          <TouchableOpacity
        ...
        <ScrollView
          ...
          {generateInsights().map((insight, idx) => (
            <View
              ...
              style={[
                styles.insightCard,
                {backgroundColor: '#232c3d', borderLeftColor: insight.color},
              ]}>
              <FontAwesome5
                name={insight.icon}
                size={24}
          ...
          <Modal visible={showHelp} transparent animationType="fade">
            <Pressable
              ...
              <View
                ...
                <Text
                  style={{color: '#b0b8c1', fontSize: 16, textAlign: 'center'}}>
                  This chart shows your total income, expenses, and net cash
                  flow for each day. Tap on the chart to see exact values.
                </Text>
              </View>
            </Pressable>
          </Modal>
            ...
            <>
              ...
              <View
                ...
                <View
                  ...
                  />
                  <Text
                    style={{color: '#b0b8c1', fontWeight: '500', fontSize: 14}}>
                    Net Cash Flow
                  </Text>
                </View>
              </View>
              <Svg
                ...
                <Defs>
                  <LinearGradient
                    ...
                    <Stop offset="100%" stopColor="#23aaff" stopOpacity={0.1} />
                  </LinearGradient>
                </Defs>
                <VictoryChart
              ...
              <View key={goal._id || idx} style={styles.goalCard}>
                <View
                  ...
                  <View style={{flex: 1}}>
                    ...
                    <Text
                      style={{color: '#b0b8c1', fontSize: 14, marginBottom: 4}}>
                    ...
                    <Text
                      style={{color: '#4caf50', fontSize: 13, marginTop: 2}}>
                  ...
                  <View
                    ...
                    <TouchableOpacity
                      onPress={() => handleGoalDelete(goal._id)}>
                      <FontAwesome5 name="trash" size={18} color="#e57373" />
                    </TouchableOpacity>
                  </View>
                </View>
                <Text style={{color: '#b0b8c1', fontSize: 13, marginTop: 6}}>
                  End Date:{' '}
                  {goal.endDate
          ...
          <View style={styles.modalOverlay}>
            <View style={styles.modalContent}>
              ...
              <TextInput
                ...
                onChangeText={v => setGoalForm(f => ({...f, targetAmount: v}))}
              ...
              <TextInput
                ...
                onChangeText={v => setGoalForm(f => ({...f, currentAmount: v}))}
                keyboardType="numeric"
              />
              <TextInput
                style={styles.modalInput}
                ...
                onChangeText={v => setGoalForm(f => ({...f, endDate: v}))}
              />
              <View
                style={{
                  flexDirection: 'row',
          ...
          <View style={styles.spendingBreakdown}>
            {spending.map((item, idx) => (
              <View key={idx} style={styles.spendingRow}>
                ...
                <Text
                  style={{color: '#0070ba', fontWeight: 'bold', fontSize: 15}}>
